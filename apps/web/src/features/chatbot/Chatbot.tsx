...
export default function Chatbot({ border, card, fg, sub, accent = "#0b6" }: Props) {
  ...
  if (import.meta && (import.meta as any).env && (import.meta as any).env.DEV) {
    console.info("[Chatbot] multiple-choice triage component mounted");
  }
...
function Footer({ reset, hasOutcome }: { reset: () => void; hasOutcome: boolean }) {
  return (
    <div
      style={{
        padding: "8px 12px",
    ...
    >
      <a href="https://111.nhs.uk/" target="_blank" rel="noreferrer" style={{ fontSize: 13 }}>
        NHS 111 online
      </a>
...
function Question({
  ...
  return (
    <div>
      ...
      {type === "yesno" && (
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => onAnswer(true)} style={btn(accent)}>
            Yes
          </button>
          <button onClick={() => onAnswer(false)} style={btnAlt()}>
            No
          </button>
        </div>
      )}
      ...
      {type === "single" && (
        <div style={{ display: "grid", gap: 8 }}>
          {options?.map((o) => (
            <button key={o.value} onClick={() => onAnswer(o.value)} style={btn(accent)}>
              {o.label}
            </button>
          ))}
        </div>
      ...
      {type === "number" && <NumberPrompt onSubmit={(n) => onAnswer(n)} />}
    </div>
  );
}
...
function NumberPrompt({ onSubmit }: { onSubmit: (n: number) => void }) {
  ...
  return (
    ...
    >
      ...
      />
      <button type="submit" style={btn("#0b6")}>
        Continue
      </button>
    </form>
  );
...
function OutcomeView({
  ...
  outcome: "CALL_999" | "A_AND_E" | "UTC_GP" | "SELF_CARE";
  accent: string;
  sub: string;
}) {
  const title =
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12, color: sub, marginTop: 12 }}>If symptoms worsen, seek urgent help.</p>
    </div>
  );
}
