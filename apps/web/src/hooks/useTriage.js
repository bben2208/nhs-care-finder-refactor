...
export function useTriage() {
    ...
    const go = useCallback((step, raw) => {
        // persist answer
        setAnswers(prev => {
            const next = { ...prev };
            switch (step.id) {
                case "who":
                    next.forWhom = raw === "ME" ? "ME" : "OTHER";
                    break;
                case "age":
                    next.age = raw;
                    break;
                ...
                default:
                    // booleans or numbers
                    if (step.type === "yesno")
                        next[step.id] = Boolean(raw);
                    if (step.type === "number")
                        next[step.id] = Number(raw);
            }
            return next;
        });
        ...
        setOutcome(prevOutcome => {
            if (prevOutcome)
                return prevOutcome; // already decided
        ...
        setStepIndex((i) => {
            ...
            if (decide999OnThis)
                return i; // keep index; UI will show outcome
            // otherwise progress
            const nextIdx = i + 1;
            ...
            if (step.id === "rfHeadInjuryRisk")
                return nextIdx;
            // end of flow? compute default outcome
            if (nextIdx >= steps.length)
                return i; // will compute outcome below (in computeOutcome)
            return nextIdx;
        });
    }, [steps]);
...
function useEffectComputeOutcome(answers, steps, stepIndex, setOutcome) {
