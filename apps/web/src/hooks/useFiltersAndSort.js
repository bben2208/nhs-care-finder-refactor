...
export function useFiltersAndSort(results, filters, sortBy, isFav) {
    return useMemo(() => {
        let arr = results.slice();
        // Filters
        if (filters.open)
            arr = arr.filter(r => r.status?.open);
        if (filters.wheelchair)
            arr = arr.filter(r => r.features?.wheelchair);
        if (filters.parking)
            arr = arr.filter(r => r.features?.parking);
        if (filters.xray)
            arr = arr.filter(r => r.features?.xray);
        if (filters.fav)
            arr = arr.filter(r => isFav(r.id));
        // Sorting
        arr.sort((a, b) => {
            switch (sortBy) {
                case "open":
